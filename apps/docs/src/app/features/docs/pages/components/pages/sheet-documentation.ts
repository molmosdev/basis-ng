...
import { Alert, Button, Sheet } from '@basis-ng/primitives';
...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-col gap-4">
      <span>Sheet is a sliding panel that appears from the left or right side of the screen.</span>
      ...
      <b-sheet [(isOpen)]="isLeftOpen" [side]="'left'" (closeSheet)="isLeftOpen.set(false)">
        <div class="flex items-center justify-center h-full">This is the left sheet.</div>
      </b-sheet>
      <b-sheet [(isOpen)]="isRightOpen" [side]="'right'" (closeSheet)="isRightOpen.set(false)">
        <div class="flex items-center justify-center h-full">This is the right sheet.</div>
      </b-sheet>
      ...
      <div
        class="border border-gray-200 dark:border-neutral-900 rounded-lg p-6 mb-6 flex flex-col gap-4 items-center"
      >
        <button b-button (click)="openCustomWidthSheet()">Open Custom Width Sheet</button>
      </div>
      <b-sheet
      ...
      >
        <div class="flex items-center justify-center h-full">This is a custom width sheet.</div>
      </b-sheet>
    </div>
    <app-steps-buttons
      [previous]="{ label: 'Select', path: '/docs/components/select' }"
