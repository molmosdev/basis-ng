...
import { Badge, Menu, MenuItemRadio, ResponsiveManager } from '@basis-ng/primitives';
      ...
      @for (route of componentsRoutes(); track route) {
        @if (route.data) {
          ...
          >
            {{ route.data['title'] }}
            @if (route.data['badge']) {
              <span b-badge class="b-variant-outlined b-size-sm">{{ route.data['badge'] }}</span>
            }
          </button>
        }
      }

      <span class="opacity-70 text-xs mb-1 pl-1 font-display-mono mt-5">Utilities</span>
